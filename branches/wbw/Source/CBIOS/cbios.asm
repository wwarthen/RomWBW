...
; MEMORY LAYOUT
;
IOBYTE:	 	.EQU 	$0003	; LOC IN PAGE 0 OF I/O DEFINITION BYTE
CDISK:	 	.EQU 	$0004	; LOC IN PAGE 0 OF CURRENT DISK NUMBER 0=A,...,15=P
;
CCP_LOC		.EQU	CPM_LOC
CCP_SIZ		.EQU	$0800
;
...
LD_CRT	.EQU	CIODEV_CONSOLE	; -> COM0:
LD_BAT	.EQU	DEV_BAT
...
#ELSE
	...
	; COPY THE ENTIRE HBIOS CONFIG BLOCK (HCB) TO $8000
	; FOR USE DURING INIT.
	LD	B,BF_SYSXCPY	; HBIOS FUNC: SYSTEM EXTENDED COPY
	...
	LD	DE,$8000	; TO TEMP LOCATION IN USR BANK
	LD	IX,$100		; COPY CONTENTS OF HCB
	RST	08		; DO IT
	
	; CAPTURE RAM DRIVE STARTING BANK
	LD	A,($8000 + HCB_BIDRAMD0)
	LD	(BNKRAMD),A
#ENDIF
...
#IFDEF PLTWBW
	; SET IOBYTE BASED ON ACTIVE DISPLAY DEVICE
	LD	A,($8000 + HCB_SERDEV)	; GET PRIMARY SERIAL DEVICE
	LD	B,A			; SAVE IN B
	LD	A,($8000 + HCB_CONDEV)	; GET CONSOLE DEVICE
	CP	B			; COMPARE
	JR	Z,INIT1			; IF SAME, LEAVE IOBYTE ALONE
	LD	A,1			; IF DIFFERENT, SET IOBYTE FOR CONSOLE = CRT:
...
;
#IFDEF PLTWBW
	LD	A,($8000 + HCB_ROMBANKS)	; ROM BANK COUNT
	LD	IX,DPB_ROM	; ADDRESS OF DPB
	CALL	MD_INIT1	; FIX IT UP
;
	LD	A,($8000 + HCB_RAMBANKS)	; RAM BANK COUNT
...
;
	; GET BOOT DEVICE/UNIT/LU INFO
	LD	DE,($8000 + HCB_BOOTVOL)	; BOOT VOLUME (DEV/UNIT, SLICE)
	LD	(BOOTVOL),DE		; D -> DEVICE/UNIT, E -> LU
;
	; SETUP THE DRVMAP STRUCTURE
	LD	HL,(BUFTOP)		; GET CURRENT BUFFER TOP
