...
;	CALL	PCF_DBG		; [1]
	...
	LD	E,DS7_WRITE	; SET SLAVE ADDRESS
	CALL	I2C_BF_START	; GENERATE START CONDITION
	JP	NZ,DS7_WTPIN
;
	LD	E,00H		; REGISTER 00
	CALL	I2C_BF_WRITE
	JP	NZ,DS7_WTPIN
;
	LD	DE,5
	...
	JP	NZ,DS7_WTPIN
;
	LD	E,0		; END TRANSACTION, RESTART BELOW
	CALL	I2C_BF_STOP
;
	LD	E,DS7_WRITE	; SET SLAVE ADDRESS
	CALL	I2C_BF_START	; GENERATE START CONDITION
...
DS7_SET3:
	...
	ADD	HL,DE
DS7_SC1:
	LD	E,(HL)		; DATA BYTE FOR THIS REGISTER
	CALL	I2C_BF_WRITE
	RET	NZ
	DEC	HL
	DJNZ	DS7_SC1
...
;
	LD	E,DS7_READ
	CALL	I2C_BF_REPSTART
	JP	NZ,DS7_WTPIN
;
	LD	E,1		; SINGLE BYTE, NACK IT NOW
	CALL	I2C_BF_READ	; A=STATUS, L=VALUE
	JP	NZ,DS7_WTPIN
	LD	E,L		; RETURN VALUE IN E
	JP	DS7_WTPIN
;
; SET NVRAM BYTE
;   C: INDEX
...
;
DS7_SETBYT:
	LD	D,E		; SAVE VALUE, E IS I2C_BF_* PARAM
;
	LD	E,DS7_WRITE
	CALL	I2C_BF_START
	JP	NZ,DS7_WTPIN
...
;
DS7_DETECT:
	LD	E,DS7_WRITE		; SLAVE ADDRESS (WRITE)
	CALL	I2C_BF_START
	PUSH	AF			; SAVE RESULT
	LD	E,0
...
; 5.	READ 8 BYTES STARTING AT ADDRESS PREVIOUSLY SET
...
;
DS7_RDC_ADDROK:
	LD	E,0		; REGISTER 0 (START OF DS1307 REGISTERS)
	CALL	I2C_BF_WRITE
	JP	NZ,DS7_WTPIN
;
	LD	E,DS7_READ	; REPEATED START, SWITCH TO READ DIRECTION
	CALL	I2C_BF_REPSTART
	JP	NZ,DS7_WTPIN
;
...
#ENDIF
;
; RETURNS Z/A=00 ON SUCCESS, NZ/A=STATUS ON FAILURE (SEE I2C_BF_* CONTRACTS)
;
DS7_WTPIN:
	PUSH	AF
	PUSH	DE		; I2C_BF_STOP USES E, CALLER'S VALUE IS IN E
	LD	E,0
	CALL	I2C_BF_STOP
	POP	DE
	POP	AF
...
;
	...
	LD	HL,DS7_CLKDATA	; HL, E IS I2C_BF_* PARAM
DS7_SC:
	LD	A,(HL)
	CP	0FFH
	JR	Z,DS7_ESC
	LD	E,A		; DATA BYTE FOR THIS REGISTER
	CALL	I2C_BF_WRITE
	JR	Z,DS7_SCC1
	INC	C		; COUNT FAILURES, KEEP WRITING THE TABLE
DS7_SCC1:
	INC	HL
	JR	DS7_SC
;
