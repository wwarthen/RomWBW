...
;	INTERRUPT DRIVEN VERSION BY PHIL SUMMERS
;__________________________________________________________________________________________________
;
; TODO:
...
KBD_INT:
	...
	IN	A,(C)
	AND	1
	RET	Z			; NZ if character available
...
;__________________________________________________________________________________________________
;
KBDQPUSH:				; ADD A CHARACTER TO THE QUEUE
	PUSH	AF
	CALL	KBDQFULL
	JR	Z,KBDQIS		; EXIT IF THE QUEUE IS FULL
;
	LD	A,(KBDQPUTX)		; CREATE QUEUE INDEX
	AND	KBDQSIZ-1
	...
	LD	(HL),A
;
	LD	HL,KBDQPUTX		; RETRIEVE THE CHARACTER FROM THE QUEUE
	INC	(HL)
	RET
;
KBDQIS:	POP	AF
...
;
;__________________________________________________________________________________________________
; REMOVE A BYTE TO FROM KEYBOARD QUEUE
;__________________________________________________________________________________________________
;
KBDQPOP:
	CALL	KBDQLEN
	...
	LD	A,(KBDQGETX)		; CREATE QUEUE INDEX
	AND	KBDQSIZ-1
	...
	LD	HL,KBDQGETX		; PLACE THE CHARACTER IN THE QUEUE
	INC	(HL)
	RET
;
...
;
	LD	B,KBD_WAITTO		; SETUP TO LOOP
KBD_GETDATA0:
	CALL	KBDQ_IST		; GET INPUT REGISTER STATUS
	...
	RET
KBD_GETDATA1:
	CALL	KBDQPOP			; GET A KEY FROM THE INTERRUPT QUEUE
;;;	LD	C,(IY+KBD_DAT)		; DATA PORT
