...
SD_CSMSK	.EQU	%00000100	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00000000	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"JUHA"
;
...
SD_CSMSK	.EQU	%00010000	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00010000	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"PPI"
#ENDIF
;
...
;SD_INVCS	.EQU	TRUE		; INVERT CS
SD_CSMSK	.EQU	%00001000	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00001000	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"UART"
#ENDIF
;
...
SD_IOBASE	.EQU	SD_OPRREG	; IOBASE
;SD_INVCS	.EQU	TRUE		; INVERT CS
SD_CSMSK	.EQU	%00001100	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00001100	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"SC"
RTCDEF		.SET	RTCDEF | SD_OPRDEF	; SET DEFAULT IN HBIOS MAINLINE
#ENDIF
;
...
;SD_INVCS	.EQU	FALSE		; INVERT CS
SD_CSMSK	.EQU	%00110000	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00000000	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"MT"
#ENDIF
;
...
SD_IOBASE	.EQU	SD_OPRREG	; IOBASE
;SD_INVCS	.EQU	FALSE		; INVERT CS
SD_CSMSK	.EQU	%00000111	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00000000	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"EPITX"
#ENDIF
;
...
;SD_INVCS	.EQU	TRUE		; INVERT CS
SD_CSMSK	.EQU	%00000011	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00000011	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"T35"
#ENDIF
;
; S100 FPGA Z80 SPI-BASED SD CARD ON ALTERNATE PORTS
;
; BASE PORT: $74
;   BASE + 0: DATA IN/OUT
;   BASE + 1: SPI CLOCK SPEED (0=LOW 4KHZ, 1=HIGH 10MHZ)
...
;   0: PRIMARY DEVICE, USE VALUE ~$01
;   1: SECONDARY DEVICE, USE VALUE ~$02
;   2: TERTIARY DEVICE, USE VALUE ~$04
;
#IF (SDMODE == SDMODE_T35ALT)		; S100 FPGA Z80 ALT PORTS
SD_IOBASE	.EQU	$74		; IOBASE
SD_DATA		.EQU	SD_IOBASE + 0	; DATA IN/OUT PORT
SD_CLKSEL	.EQU	SD_IOBASE + 1	; CLOCK SPEED SELECT PORT
SD_SELSTAT	.EQU	SD_IOBASE + 2	; DEVICE SELECT PORT (W) / STATUS (R)
SD_ACTION	.EQU	SD_IOBASE + 3	; INITIATE R/W ACTION VIA IN/OUT
;
SD_DEVMAX	.EQU	3		; NUMBER OF PHYSICAL UNITS (SOCKETS)
SD_OPRREG	.EQU	SD_SELSTAT	; SELECT/STATUS PORT
SD_OPRDEF	.EQU	$FF		; QUIESCENT STATE
SD_CS0		.EQU	%00000001	; PRIMARY DEVICE SELECT BIT
SD_CS1		.EQU	%00000010	; SECONDARY DEVICE SELECT BIT
SD_CS2		.EQU	%00000100	; TERTIARY DEVICE SELECT BIT
;SD_INVCS	.EQU	TRUE		; INVERT CS
SD_CSMSK	.EQU	%00000111	; CHIP SELECT BITS MASK
SD_CSINV	.EQU	%00000111	; CHIP SELECT BITS INVERSION MASK
		DEVECHO	"T35ALT"
#ENDIF
;
...
;
SD_CFGSIZ	.EQU	15		; SIZE OF CFG TBL ENTRIES
;
...
SD_CFG_CS	.EQU	12		; BIT PATTERN TO SELECT CHIP
SD_CFG_CSMSK	.EQU	13		; CHIP SELECT BITS MASK
SD_CFG_CSINV	.EQU	14		; CHIP SELECT INVERSION MASK
;
SD_CFGTBL:
	; DEVICE 0
	...
	.DW	0,0			; CURRENT LBA
	.DB	SD_CS0			; BIT PATTERN TO SELECT CHIP
	.DB	~SD_CSMSK		; CHIP SELECT BITS MASK (INVERTED!)
	.DB	SD_CSINV		; CHIP SELECT INVERSION MASK
;
#IF (SD_DEVCNT >= 2)
	; DEVICE 1
	...
	.DW	0,0			; DEVICE CAPACITY
	.DW	0,0			; CURRENT LBA
	.DB	SD_CS1			; BIT PATTERN TO SELECT CHIP
	.DB	~SD_CSMSK		; CHIP SELECT BITS MASK (INVERTED!)
	.DB	SD_CSINV		; CHIP SELECT INVERSION MASK
#ENDIF
;
#IF (SD_DEVCNT >= 3)
	...
	.DW	0,0			; DEVICE CAPACITY
	.DW	0,0			; CURRENT LBA
	.DB	SD_CS2			; BIT PATTERN TO SELECT CHIP
	.DB	~SD_CSMSK		; CHIP SELECT BITS MASK (INVERTED!)
	.DB	SD_CSINV		; CHIP SELECT INVERSION MASK
#ENDIF
;
...
#ENDIF
;
#IF ((SDMODE == SDMODE_T35) | (SDMODE == SDMODE_T35ALT))
...
;
SD_SELECT:
	LD	A,(SD_OPRVAL)		; GET OPR REG SHADOW VALUE
	AND	(IY+SD_CFG_CSMSK)	; CLEAR CS BITS
	OR	(IY+SD_CFG_CS)		; APPLY NEW CS BITS
	XOR	(IY+SD_CFG_CSINV)	; INVERT CS BITS AS NEEDED
	LD	(SD_OPRVAL),A		; UPDATE OPR REG SHADOW VALUE
	OUT	(SD_OPRREG),A		; WRITE NEW VALUE TO OPR REG
	RET				; DONE
;
; DESELECT CARD
;
