...
;------------------------------------------------------------------------------
;
; ALL COMMAND RESPONSES START WITH R1 DEFINED AS FOLLOWS:
;
;	 R1 RESPONSE CODE:
;
;	   7   6   5   4   3   2   1   0
...
;
; SD DRIVER INITIALIZATION ENTRY POINT
;
SD_INIT:
	PRTS("SD:$")
;
...
SD_INITUNIT:
	...
	JP	NZ,SD_PRTERR			; EXIT VIA PRINT STATUS
;
...
;
	...
	CALL	SD_PRTPREFIX			;
	JP	NZ,SD_PRTSTAT			; EXIT VIA PRINT STATUS
;
	; PRINT CARD TYPE
	PRTS(" TYPE=$")
	...
	CP	SD_TYPEMMC
	JR	Z,SD_INITUNIT1
	LD	DE,SD_STRTYPESDSC
	CP	SD_TYPESDSC
	JR	Z,SD_INITUNIT1
	LD	DE,SD_STRTYPESDHC
	CP	SD_TYPESDHC
	JR	Z,SD_INITUNIT1
	LD	DE,SD_STRTYPESDXC
	CP	SD_TYPESDXC
	JR	Z,SD_INITUNIT1
	LD	DE,SD_STRTYPEUNK
SD_INITUNIT1:
...
;
	...
	JR	Z,SD_INITUNIT3			; NOPE, BYPASS
	CALL	PC_SPACE			; SEPARATOR
	CALL	NZ,SD_PRTSTAT
	; WE DON'T EXIT HERE SINCE WRITE PROTECTION IS NOT A FATAL ERROR
;
SD_INITUNIT3:
...
;
; SD DRIVER FUNCTION DISPATCH ENTRY POINT
;
SD_DISPATCH:
...
SD_READ:
	...
	JP	SD_PRTERR			; RETURN VIA PRINT ERROR
;
SD_WRITE:
	; WRITE A SECTOR
	CALL	SD_CHKWP			; CHECK FOR WRITE PROTECT
	JP	NZ,SD_PRTERR			; IF PROTECTED, EXIT VIA PRINT ERROR
...
SD_MEDIA:
	...
	CALL	SD_PRTERR			; FAILURE, PRINT ERROR
...
#IF (SDTRACE >= 2)
	CALL	Z,SD_PRTPREFIX
#ENDIF
	JP	Z,SD_NOMEDIA			; NO MEDIA DETECTED IF ZERO
#ENDIF
;
...
SD_SECTIO:
	...
	LD	A,C				; LOAD SD CARD COMMADN BYTE
	CALL	SD_INITCMD			; SETUP COMMAND BUFFER
...
;
;==================================================================================================
;   SD DISK DRIVER COMMAND PROCESSING
;==================================================================================================
;
; PUT CARD IN IDLE STATE
;
...
#IF (SDTRACE >= 2)
	CALL	NZ,SD_PRTPREFIX
#ENDIF

	JP	NZ,SD_WRTPROT			; HANDLE AS ERROR
...
#ENDIF
	...
	RET
;
; PRINT STATUS IF AN ERROR HAS OCCURRED
;
SD_PRTERR:
#IF (SDTRACE >= 1)
	;RET	Z				; IF NO ERROR, GET OUT
	PUSH	AF				; SAVE ACCUM/FLAGS
	CALL	SD_PRTPREFIX			; PRINT UNIT PREFIX
	CALL	PC_SPACE			; FORMATTING
	CALL	SD_PRTSTAT			; PRINT STATUS
...
SD_PRTSTAT:
	...
	PUSH	DE
	PUSH	HL
	SD_DPTR(SD_STAT)			; GET ADR OF STATUS IN HL, AF TRASHED
	LD	A,(HL)				; GET STATUS
	OR	A
	LD	DE,SD_STRSTOK
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTNOTRDY
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTRDYTO
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTINITTO
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTCMDTO
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTCMDERR
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTDATAERR
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTDATATO
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTCRCERR
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTNOMEDIA
	JR	Z,SD_PRTSTAT1
	INC	A
	LD	DE,SD_STRSTWRTPROT
	JR	Z,SD_PRTSTAT1
	LD	DE,SD_STRSTUNK
;
SD_PRTSTAT1:
...
SD_STRCID	.TEXT	" CID =$"
SD_STRSCR	.TEXT	" SCR =$"
SD_STRSDTYPE	.TEXT	" SD CARD TYPE ID=$"
;
SD_STRSTOK	.TEXT	"OK$"
SD_STRSTNOTRDY	.TEXT	"NOT READY$"
SD_STRSTRDYTO	.TEXT	"READY TIMEOUT$"
SD_STRSTINITTO	.TEXT	"INITIALIZATION TIMEOUT$"
SD_STRSTCMDTO	.TEXT	"COMMAND TIMEOUT$"
SD_STRSTCMDERR	.TEXT	"COMMAND ERROR$"
SD_STRSTDATAERR .TEXT	"DATA ERROR$"
SD_STRSTDATATO	.TEXT	"DATA TIMEOUT$"
SD_STRSTCRCERR	.TEXT	"CRC ERROR$"
SD_STRSTNOMEDIA .TEXT	"NO MEDIA$"
SD_STRSTWRTPROT .TEXT	"WRITE PROTECTED$"
SD_STRSTUNK	.TEXT	"UNKNOWN$"
SD_STRTYPEUNK	.TEXT	"UNK$"
SD_STRTYPEMMC	.TEXT	"MMC$"
SD_STRTYPESDSC	.TEXT	"SDSC$"
SD_STRTYPESDHC	.TEXT	"SDHC$"
SD_STRTYPESDXC	.TEXT	"SDXC$"
;
;==================================================================================================
; SD DISK DRIVER - DATA
;==================================================================================================
...
;
; UNIT SPECIFIC DATA STORATE
;
SD_UNIT		.DB	-1			; ACTIVE UNIT, INIT TO -1 TO INDICATE NOTHING SELECTED
