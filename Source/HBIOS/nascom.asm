...
FNOFST: LD      B,0		; Get address of function
	...
	CP	2*(ZPOINT-ZSGN)	; Adjusted "POINT" token?	; ECB-VDU
	JP	Z,POINTB	; Yes - Do "POINT" (not POINTB)	; ECB-VDU
...
;	POP	AF
	...
	OR	10000000B	; STACK = Character Columns
				; A = bit mask
	POP	BC
	LD	B,C
	PUSH    AF		; Save mask
...
;
; Convert coordinate line A, column on stack
;
VDULINS	.EQU	25
VDUCOLS	.EQU	80
VDUSIZE	.EQU	(VDULINS*VDUCOLS)+VDUCOLS

; on entry STACK = Return Address, Character Column, Bit Mask. A = Character Line
...
ADD80X: ADD     HL,DE           ; Add line
        DJNZ    ADD80X          ; SIXTY FOUR TIMES!!!
        RET
...
XYPOS:  POP     BC		; Get return address
	...
	LD	A,L
	SRL	A		; Store it in the carry
	LD	L,A		; Make HL a character column

	SBC	A,A		; if carry =0, a=0, if carry =1 a=ff
	INC	A		; if carry was =0, a=1, carry =1 a=0
	INC	A		; if carry was =0, a=2, carry =1 a=1
