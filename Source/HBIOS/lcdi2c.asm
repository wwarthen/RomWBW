...
LCD_MESSAGE:
	...
	LD	H,(HL)			; GET MSB
	LD	L,A			; GET LSB
	PUSH	HL			; SAVE STRING POINTER, LCD_BUSSTART CLOBBERS E
;
	CALL	LCD_BUSSTART
	JR	NZ,LCD_MESSAGE_NOACK	; NO ACK, BAIL
	POP	DE			; RECOVER STRING POINTER INTO DE
	...
	JR	LCD_MESSAGE_STOP
LCD_MESSAGE_NOACK:
	POP	HL			; DISCARD SAVED POINTER, NOT USING IT
LCD_MESSAGE_STOP:
	LD	E,0
	CALL	I2C_BF_STOP
	XOR	A			; SIGNAL SUCCESS
...
;
LCD_DETECT:
	LD	E,LCDI2CADDR << 1	; SLAVE ADDRESS (WRITE)
	CALL	I2C_BF_START
	PUSH	AF			; SAVE RESULT
	LD	E,0
	CALL	I2C_BF_STOP		; RELEASE BUS EITHER WAY
	POP	AF
	JR	NZ,LCD_DETECT1
;
	; LCD PRESENT
...
; RETURNS Z IF READY TO SEND BYTES, NZ IF NO ACK OR BUS BUSY
; CALLER SHOULD STILL CALL I2C_BF_STOP (E=0) TO RELEASE THE BUS
;
LCD_BUSSTART:
	LD	E,LCDI2CADDR << 1
	JP	I2C_BF_START
...
LCD_SENDBUF1:
	LD	E,(HL)
	CALL	I2C_BF_WRITE	; PUT DATA ON BUS, WAIT FOR ACK
	JR	Z,LCD_SENDBUF2
