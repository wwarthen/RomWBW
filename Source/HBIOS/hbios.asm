...
SYS_FREE:
	...
	EX	DE, HL			; ASSIGN TO DE THE HEADER ADDR OF
	DEC	DE			; BLOCK SUPPLIED IN HL
	DEC	DE
	DEC	DE
	DEC	DE

	LD	HL, (CB_HEAP)		; SEARCH FOR THIS BLOCK IN THE HEAP

SYS_FREE1:
	LD	A, H			; TEST IF HL EQUAL TO BLOCK TO BE FREE'D
	CP	D
	JR	NZ, SYS_FREE3		; NOT THIS BLOCK
	LD	A, L
	CP	E
	JR	NZ, SYS_FREE3		; NOT THIS BLOCK

					; FOUND THE BLOCK TO BE FREE
	PUSH	HL			; SAVE THE BLOCK'S HEADER POINTER

	CALL	SYS_HPNXTBLK		; IS THIS THE LAST BLOCK
	CALL	SYS_HPISEND
	JR	NZ, SYS_FREE2		; NO - CANT FREE IT

	POP	HL			; FOUND REQUEST BLOCK AND ITS AT END OF HEAP
	LD	(CB_HEAPTOP), HL	; MOVE HEAP END POINTER DOWN TO RELEASE THIS BLOCK
	JR	SYS_FREEOK

SYS_FREE2
	POP	HL			; RESTORE STACK STATE

SYS_FREEOK
	POP	BC			; RETURN SUCCESS
	POP	DE
	RET_SUCCESS

SYS_FREE3:				; MOVE HL TO NEXT BLOCK
	CALL	SYS_HPNXTBLK
	CALL	SYS_HPISEND		; CONTINUE LOOP
	JR	NZ, SYS_FREE1		; UNLESS ITS THE LAST BLOCK
...
; MOVE HL TO NEXT BLOCK IN HEAP
; HL POINTS TO HEADER OF BLOCK - WITH FIRST WORD IS SIZE OF BLOCK (NO INCLUDING 4 BYTE HEADER)
; RETURNS :
;	HL IS NEXT BLOCK START ADDRESS
; 	BC EQUALS SIZE OF THE INCOMING BLOCK
SYS_HPNXTBLK:
	LD	C, (HL)
	INC	HL
	LD	B, (HL)
...
; TEST IF HL POINTS TO HEAP END (MOVE PAST THE LAST BLOCK)
; SET Z FLAG IF AT HEAP END
SYS_HPISEND:
	LD	A, (CB_HEAPTOP)
	CP	L
	RET	NZ
