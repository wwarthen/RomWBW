...
;
#IF (FPLED_ENABLE)
  ; THIN WRAPPER AROUND FP_SETLEDS
...
HB_DSKREAD_NEW:
	...
	POP	HL			; RECOVER HL
;
	; CHECK DEVICE STATUS, PERFORM A RESET IF IN ERROR STATUS
	LD	A,(IY+CFG_STATUS)	; GET STATUS
	OR	A			; SET FLAGS
	CALL	NZ,HB_DSKRESET		; PERFORM DEVICE RESET
	JP	NZ, DSK_SHOWERR		; IF STILL ERROR, EXPOSE & RETURN
;
	LD	(HB_DSKCMD),BC		; SAVE HBIOS FUNC & UNIT
	CALL	HB_DSKACT_NEW		; DISK ACTIVITY NOTIFICAITON
;
...
DSK_SHOWERR:
	...
	JR	Z,DSK_SHOWERR1		; HANDLE WRITE
	PRTS(" [READ$")
	JR	DSK_SHOWERR2
DSK_SHOWERR1:
	PRTS(" [WRITE$")
...
;
;--------------------------------------------------------------------------------------------------
; CALL CURRENT DEVICE RESET FUNCTION DIRECTLY
;--------------------------------------------------------------------------------------------------
;
HB_DSKRESET:
	PUSH	BC
	PUSH	DE
	PUSH	HL
	LD	L,(IY+CFG_DISPTBL+0)	; LSB OF DISPATCH TABLE
	LD	H,(IY+CFG_DISPTBL+1)	; MSB OF DISPATCH TABLE
	INC	HL			; BUMP TO
	INC	HL			; ... RESET FUNC ADR
	LD	A,(HL)			; LSB TO ACCUM
	INC	HL			; BUMP TO MSB
	LD	H,(HL)			; LOAD MSB
	LD	L,A			; LOAD LSB
	CALL	JPHL			; DO IT
...
;
;--------------------------------------------------------------------------------------------------
; SEEK TO LBA ON CURRENT DISK DEVICE
;--------------------------------------------------------------------------------------------------
;
HB_DSKSEEKLBA:
	BIT	7,D			; CHECK FOR LBA FLAG
	CALL	Z,HB_CHS2LBA		; CLEAR MEANS CHS, CONVERT TO LBA
	LD	(IY+CFG_DSKADR+0),L	; SAVE NEW LBA
	LD	(IY+CFG_DSKADR+1),H	; ...
	LD	(IY+CFG_DSKADR+2),E	; ...
	LD	(IY+CFG_DSKADR+3),D	; ...
	XOR	A			; SIGNAL SUCCESS
	RET				; AND RETURN
;
;--------------------------------------------------------------------------------------------------
; GET DISK CAPACITY OF CURRENT DISK DEVICE
;--------------------------------------------------------------------------------------------------
;
HB_DSKCAP:
...
;
;--------------------------------------------------------------------------------------------------
; GET DISK GEOMETRY OF CURRENT DISK DEVICE
;--------------------------------------------------------------------------------------------------
;
HB_DSKGEOMLBA:
	; FOR LBA, WE SIMULATE CHS ACCESS USING 16 HEADS AND 16 SECTORS
	; RETURN HS:CC -> DE:HL, SET HIGH BIT OF D TO INDICATE LBA CAPABLE
	; THE CALL TO HB_DSKCAP WILL SET THE DEVICE STATUS (REG A), SO THAT IS
	; JUST PASSED THRU.
	CALL	HB_DSKCAP		; GET TOTAL BLOCKS IN DE:HL, BLOCK SIZE TO BC
...
;
;--------------------------------------------------------------------------------------------------
; INCREMENT LBA OF CURRENT DISK DEVICE
;--------------------------------------------------------------------------------------------------
;
...
; IS A WRITE.
;
;   IY: ADDRESS OF DISK DRIVER CONFIG BLOCK
;   ALL REGISTERS PRESERVED
;
HB_DSKACT_NEW:
;
	...
	PUSH	HL
;
	; COPY CURRENT DISK ADDRESS TO GLOBAL STORAGE
	LD	A,CFG_DSKADR		; OFFSET TO CUR DISK ADDRESS
	CALL	LDHLIYA			; ADDRESS OF ADR TO HL
	CALL	LD32			; LOAD IT TO DE:HL
	BIT	7,D			; LBA OR CHS?
	JR	NZ,HB_DSK_ACT_NEW1	; IF NZ, LBA, SKIP AROUND
	EX	DE,HL			; IS CHS, SO MAP HSCC --> CCHS
HB_DSK_ACT_NEW1:
	LD	BC,HB_DSKADR		; SET BC TO GLOBAL DISK ADR
	CALL	ST32			; SAVE IT
;
	; FIRE DISK ACTIVITY EVENT
	LD	B,BF_DSKYEVENT
	LD	C,DSKY_EVT_DSKACT
	LD	A,(DSKY_DISPACT)
	OR	A
...
;
;--------------------------------------------------------------------------------------------------
;   I2C DEVICE DISPATCHER
;--------------------------------------------------------------------------------------------------
;
; ROUTE CALL TO THE ACTIVE I2C BUS-MASTER
; AT THIS TIME THERE IS ONE LOGICAL I2C BUS.
;   B: FUNCTION
;   C: UNIT NUMBER (ALWAYS 0 AT THIS TIME)
;
I2C_DISPATCH:
	PUSH	IY			; SAVE INCOMING IY
;
	LD	IY, I2C_TBL		; POINT IY TO START OF I2C TABLE
	CALL	HB_DISPCALL		; GO TO GENERIC API CALL CODE
;
	POP	IY			; RESTORE IY
	RET				; AND DONE
...
; PRESENCE PROBE COVERS BOTH, SAME I2C-ACK TECHNIQUE AS LCD_DETECT IN
; lcdi2c.asm.
;
FP_DETECT:
...
;
	CALL	I2C_WAIT_FOR_BB
	JR	NZ,FP_DETECTZ		; BUS STUCK BUSY, GIVE UP QUIETLY
	LD	A,FP_I2CADR_W
	CALL	I2C_START_ADDR
	CALL	I2C_WAIT_FOR_ACK
	PUSH	AF
	CALL	I2C_STOP
	POP	AF
	JR	NZ,FP_DETECTZ		; NO ACK, TCA9555 NOT PRESENT
;
	CALL	FP_TCA_INIT		; P0=INPUT (SWITCHES), P1=OUTPUT (LEDS)
	JR	NZ,FP_DETECTZ		; CONFIG WRITE FAILED, TREAT AS NOT PRESENT
;
	LD	D,TRUE
	LD	E,TRUE
	JR	FP_DETECTZ
...
;
  ...
  #IF (FP_USE_I2C)
	...
	JR	NZ,FP_SETLEDS1
	CALL	I2C_WAIT_FOR_BB
	JR	NZ,FP_SETLEDS1		; BUS STUCK BUSY, GIVE UP QUIETLY
	LD	A,FP_I2CADR_W
	CALL	I2C_START_ADDR
	CALL	I2C_WAIT_FOR_ACK
	JR	NZ,FP_SETLEDS_STOP	; NO ACK, MODULE GONE, STILL RELEASE BUS
	LD	A,FP_TCA_OUT1		; SELECT OUTPUT PORT 1 (LEDS)
	CALL	I2C_PUTBYTE_ACK
...
FP_GETSWITCHES:
  ...
  #IF (FP_USE_I2C)
	CALL	I2C_WAIT_FOR_BB
	JR	NZ,FP_GETSWITCHES_NONE	; BUS STUCK BUSY, RETURN ZERO
	LD	A,FP_I2CADR_W
	CALL	I2C_START_ADDR
	CALL	I2C_WAIT_FOR_ACK
	JR	NZ,FP_GETSWITCHES_STOP	; NO ACK, MODULE GONE
	LD	A,FP_TCA_IN0		; SELECT INPUT PORT 0 (SWITCHES)
	CALL	I2C_PUTBYTE_ACK
	JR	NZ,FP_GETSWITCHES_STOP
	CALL	I2C_REPSTART		; SWITCH TO READ DIRECTION, SAME DEVICE
	LD	A,FP_I2CADR_R
	CALL	I2C_PUTBYTE_ACK
	JR	NZ,FP_GETSWITCHES_STOP
	CALL	I2C_PREPNACK		; SINGLE BYTE READ, NACK IMMEDIATELY
	CALL	I2C_WAIT_FOR_PIN
	JR	NZ,FP_GETSWITCHES_STOP
	CALL	I2C_GETBYTE		; A = SWITCH BYTE
	PUSH	AF
	CALL	I2C_STOP
	POP	AF
