...
I2CBIT_INIT:
	...
	CALL	PRTHEXBYTE
	CALL	PC_SPACE
	CALL	I2C_ENSURE_INIT
	RET	Z				; SUCCESS
	LD	HL,(I2CBIT_FAIL_MSG)		; PRINT THE RECORDED FAILURE REASON
	CALL	PRTSTR
	RET
;
I2CBIT_INITDEV:
	LD	A,I2CBIT_QUIES
	OUT	(I2CBIT_BASE),A
	CALL	DELAY			; CLOBBERS A, ALWAYS RETURNS WITH A=0
;
	LD	A,I2CBIT_QUIES
	RES	I2CBIT_SDA,A		; SDA LOW, SCL STAYS HIGH
	OUT	(I2CBIT_BASE),A
	IN	A,(I2CBIT_BASE)
	BIT	I2CBIT_SDA,A
...
I2CBIT_INITDONE:
	.DB	FALSE
;
; FIXME ONLY A SUCCESSFUL I2CBIT_INITDEV IS CACHED, A FAILED FIRST ATTEMPT
; MUST NOT LATCH FOREVER.
;
I2C_ENSURE_INIT:
	LD	A,(I2CBIT_INITDONE)
	OR	A
	JR	Z,I2C_ENSURE_INIT1	; NOT DONE YET
	XOR	A			; ALREADY SUCCEEDED, FORCE Z
	RET
I2C_ENSURE_INIT1:
	CALL	I2CBIT_INITDEV
	RET	NZ			; FAILED, LEAVE UNCACHED SO WE RETRY NEXT CALL
	LD	A,TRUE
	LD	(I2CBIT_INITDONE),A
	XOR	A			; ENSURE Z SET
	RET
;
;-----------------------------------------------------------------------------
...
I2C_START:
	...
	OUT	(I2CBIT_BASE),A
	LD	A,D			; SHIFT THE STASHED ADDRESS BYTE
	JP	I2CBIT_SHIFTOUT
;
;-----------------------------------------------------------------------------
...
I2C_WAIT_FOR_BB:
	...
	RET
;-----------------------------------------------------------------------------
; NON-BLOCKING BUS-BUSY CHECK
; RETURN WITH A/NZ IF FREE, A=0/Z IF BUSY.
;
I2C_CHECK_BB:
	LD	A,1
	OR	A		; FORCE NZ
...
;
;-----------------------------------------------------------------------------
; BITBANG HAS NO PCF8584-STYLE DUMMY BYTE, JUST READ AND RETURN
;
I2C_GETBYTE:
;
...
I2CBIT_INIERR:
	...
	JR	I2CBIT_PRTERR
;
; INIT-FAILURE PATHS RECORD THE REASON FOR I2CBIT_INIT TO PRINT LATER.
;
I2C_RECORDFAIL:
	LD	(I2CBIT_FAIL_MSG),HL	; STASH WHICH MESSAGE APPLIES
	POP	HL			; RESTORE CALLER'S HL
	LD	A,ERR_NOHW
	LD	(I2C_FAIL_FLAG),A
	OR	$FF			; FORCE NZ, THIS IS THE FAILURE PATH
	RET
;
I2CBIT_ACKERR:
	PUSH	HL
